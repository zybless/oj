...
const getters = {
  ...
  contestMenuDisabled: (state, getters) => {
    ...
    if (state.contest.userDuration &&
      state.contest.userDuration > 0 && !state.startTime) {return true}
  ...
  //新不可提交逻辑
  newLogicDisabled:(state, getters)=>{
    if (
      state.contest.userDuration &&
      state.contest.userDuration > 0
    ){
      if(getters.isContestAdmin){
        return false
      }
      if(state.startTime){
        let durationMs = state.now.diff(state.startTime, "seconds");
        let duration = state.contest.userDuration;
        let countdown2 = duration - durationMs;
        if (countdown2 > 0) {
          return false;
        }
        return true;
  ...
  buttonVisible: (state, getters) => {
    ...
    return (
      ...
      !getters.isContestAdmin && 
      (state.contest.userDuration &&
        state.contest.userDuration > 0 &&!state.startTime)
    );
  },
  ...
  countdown2: (state, getters) => {
    if (
      ...
      let countdown2 = duration - durationMs;
      if (countdown2 > 0) {
        return time.secondFormat(countdown2);
      }
      return "";
    } else {
      return "";
