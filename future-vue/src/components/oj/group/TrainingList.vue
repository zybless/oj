<template>
  <div>
    <vxe-table
      ...
      v-if="!editPage"
    >
      <vxe-table-column field="id" width="60" title="ID"> </vxe-table-column>
      <vxe-table-column field="rank" width="60" :title="$t('m.Order_Number')">
      </vxe-table-column>
      <vxe-table-column
        field="title"
        min-width="150"
        ...
        <template v-slot="{ row }">
          ...
          >
            <el-button
              icon="el-icon-tickets"
              size="mini"
              @click.native="goTrainingExamList(row.id)"
              type="success"
            >
            </el-button>
          </el-tooltip>
    ...
    ></Training>
    <ExamList ref="examList"></ExamList>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    goTrainingExamList(trainingId) {
      this.$refs.examList.open(trainingId);
    },
