      ...
      >
        <template v-slot="{ row }">
          <el-select
            ...
            size="small"
            :disabled="!isAdmin && !isGroupOwner && row.auth >= userAuth"
          >
            <el-option
              :label="$t('m.Applying')"
              :value="1"
              :disabled="!isAdmin && !isGroupOwner && 1 >= userAuth"
            ></el-option>
            <el-option
              :label="$t('m.Refused')"
              :value="2"
              :disabled="!isAdmin && !isGroupOwner && 2 >= userAuth"
            ></el-option>
            <el-option
              :label="$t('m.General_Member')"
              :value="3"
              :disabled="!isAdmin && !isGroupOwner && 3 >= userAuth"
            ></el-option>
            <el-option
              :label="$t('m.Group_Admin')"
              :value="4"
              :disabled="!isAdmin && !isGroupOwner && 4 >= userAuth"
            ></el-option>
            <el-option
              :label="$t('m.Group_Root')"
              :value="5"
              :disabled="!isAdmin && !isGroupOwner && 5 >= userAuth"
            ></el-option>
          </el-select>
        </template>
      </vxe-table-column>
