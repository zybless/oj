  ...
  <div>
    <el-card>
      ...
      >
        ...
        <vxe-table-column field="isVip" title="是否vip" min-width="60">
          <template v-slot="{ row }">
            <span v-if="row.isVip">是</span>
            <span v-else>否</span>
          </template>
        </vxe-table-column>
        <vxe-table-column
        ...
        <vxe-table-column :title="$t('m.Option')" min-width="200">
          <template v-slot="{ row }">
            ...
            <el-tooltip effect="dark" content="添加该用户为vip" placement="top">
              <el-button
                icon="el-icon-edit-outline"
                size="mini"
                @click.native="editVip('add', row.uid)"
                type="primary"
                :disabled="row.isVip"
              >
              </el-button>
            </el-tooltip>
            <el-tooltip effect="dark" content="取消该用户的vip" placement="top">
              <el-button
                icon="el-icon-edit-outline"
                size="mini"
                @click.native="editVip('remove', row.uid)"
                type="primary"
                :disabled="!row.isVip"
              >
              </el-button>
    ...
    >
      ...
      >
        <el-row :gutter="10">
          ...
          <el-col :span="24">
            <el-form-item label="OI币" prop="oiCoin">
              <el-input-number v-model="selectUser.oiCoin" :min="0" :precision="0" size="small"></el-input-number>
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
import { addVip, removeVip } from "@/common/newApi/user/index";
export default {
  name: "user",
  data() {
  ...
  methods: {
    ...
    openUserDialog(row) {
      ...
      this.selectUser.oiCoin = row.oiCoin;
    },
    // 获取用户列表
    getUserList(page) {
