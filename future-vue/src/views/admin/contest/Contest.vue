      ...
      <el-form label-position="top">
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            <el-form-item label="vip专属" required>
              <el-select
                v-model="contest.isVip"
              >
          ...
          <el-col :span="24">
            <el-form-item label="单独计时时长（分钟）" required>
              <el-input-number
                v-model="contest.userDuration"
                :step="1"
                :precision="0"
                :min="0"
                :max="1440"
        ...
        <el-row>
          ...
          <el-col>
            <el-form-item :label="$t('m.Star_User_UserName')">
              <el-tag
...
export default {
  ...
  watch: {
    ...
    fileList(newval) {
      if (newval.length > 0) {
        this.contest.image = newval[0].url;
      } else {
        this.contest.image = "";
      }
    },
  ...
  methods: {
    ...
    getContestByCid() {
      api
        ...
        .then((res) => {
          ...
          this.contest = data;
          this.fileList = [];
          if (this.contest.image) {
            this.fileList = [{ url: this.contest.image }];
          }
          if (this.contest && this.contest.userDuration) {
            this.contest.userDuration = this.contest.userDuration / 60;
          }
          this.changeDuration();
          // 封榜时间转换
    ...
    saveContest() {
      ...
      if (data.userDuration) {
        data.userDuration = data.userDuration * 60;
      }
      api[funcName](data)
        .then((res) => {
    ...
    async insertEvent(row) {
      ...
      this.contest.awardConfigList = this.contest.awardConfigList.concat(
        insertRecords
      );
