<template>
    <div>
      <el-card>
        <div slot="header">
          <span class="panel-title home-title">{{ $t("m.Choose_exam_List") }}</span>
        ...
        >
          ...
          <vxe-table-column min-width="120" title="开始时间">
            <template v-slot="{ row }">
              {{ row.startTime | localtime }}
            </template>
          </vxe-table-column>
          <vxe-table-column min-width="120" title="结束时间">
            <template v-slot="{ row }">
              {{ row.endTime | localtime }}
            </template>
          </vxe-table-column>
  ...
  import { getChooseExamList, deleteChooseExam } from "@/common/newApi/chooseExam/index";
