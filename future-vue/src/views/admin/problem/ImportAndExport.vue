<template>
  <div>
    <el-card v-if="isSuperAdmin == true">
      <div slot="header">
    ...
    <el-card style="margin-top:15px">
      ...
      <div style="display: flex;margin-top: 10px;">
        <el-input placeholder="请输入起始编号" v-model="id2" style="width: 100px;margin-right: 10px;"></el-input>
        <el-input-number placeholder="请输入题目数" v-model="num2" :min="0" :step="1" style="width: 180px;margin-right: 10px;"></el-input-number>
        <el-upload
          ref="hoj2"
          action=""
          name="file"
          ...
          :with-credentials="true"
          :limit="10000"
          :on-change="onFile5Change"
          :auto-upload="false"
          :on-success="uploadSucceeded"
          :on-error="uploadFailed"
          multiple
          :data="{id:id2,num:num2}"
        >
          <el-button
            size="small"
            :loading="loading.hoj2"
            type="primary"
            slot="trigger"
            icon="el-icon-folder-opened"
            >选择多个文件</el-button
          ...
          <el-button
            ...
            size="small"
            type="success"
            @click="submitUpload('hoj2')"
            :loading="loading.hoj2"
            :disabled="!fileList5.length"
            icon="el-icon-upload"
            >{{ $t("m.Upload") }}</el-button
          >
        </el-upload>
...
import {batchUploadProblems} from "@/common/newApi/ojproblem/index"
export default {
  ...
  methods: {
    ...
    submitUpload(ref) {
      ...
      if(ref=='hoj2'){
        if (this.fileList5.length === 0) {
          myMessage.warning('请选取文件后再上传')          
        }else{
           // 下面的代码将创建一个空的FormData对象:
           const formData = new FormData()
          // 你可以使用FormData.append来添加键/值对到表单里面；
          // let tmpList = []
          console.log("fileList5", this.fileList5)
          this.fileList5.forEach((file) => {
            // tmpList.push(file.raw)
            formData.append('file', file.raw)
          })
          // formData.append('file', tmpList)
          // 添加自定义参数，不传可删除
          formData.append('id', this.id2)
          formData.append('num', this.num2)
          batchUploadProblems(formData).then(res => {
            myMessage.success(this.$i18n.t("m.Upload_Problem_Succeeded"));
            this.getProblems();
            this.fileList5 = []
          }).catch(err=>{
            this.loading.hoj2 = false;
            this.getProblems();
          })
        }
      }else{
    ...
    onFile5Change(file, fileList) {
      this.fileList5 = fileList.slice(-fileList.length);
    },
