    ...
    <el-card>
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          >
            <el-form-item
              :label="$t('m.Solution_Unlock_Ratio') + '(%)'"
              required
            >
              <el-input-number
                v-model="problem.solutionUnlockRatio"
                :precision="2"
                :step="1"
                :min="0"
                :max="100"
        ...
        >
          ...
          <div v-show="problem.isUploadCase">
            ...
            <el-col :span="12" >
              总分：<el-input style="width: 100px;margin-right: 10px;" v-model="totalScore" size="small"></el-input><el-button size="small" @click="autoScore">自动赋分</el-button>
            </el-col>
            <el-col :span="24">
              <vxe-table
                ref="xTable"
...
export default {
  ...
  methods: {
    autoScore(){
      if(this.problem.testCaseScore){
        let len = this.problem.testCaseScore.length
        if(len>0){
          let large = this.totalScore% len
          let item = Math.floor(this.totalScore / len);
          for(let i = 0;i<large;i++){
            this.problem.testCaseScore[i].score = item +1
          }
          for(let i=large;i<len;i++){
            this.problem.testCaseScore[i].score = item
          }
        }
      }
    },
    init() {
      ...
      if (this.mode === "edit") {
        ...
        // }
        this.problemLanguages = ['C++14(O2)', 'Python3']
      }
    },
