  ...
  <div style="margin-top:5px">
    <el-card shadow>
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="编程题记录" name="first">
          <vxe-table
            round
            border
            auto-resize
                ...
                <template v-if="row.submissionInfo[problem.problemId]">
                  <el-tooltip effect="dark" placement="top">
                    <div slot="content">
                      {{
                        JUDGE_STATUS[
                          row.submissionInfo[problem.problemId].status
                        ].name
                      }}
                    </div>
                    <span
          ...
          <Pagination
            ...
            @on-page-size-change="getTrainingRankData"
            :layout="'prev, pager, next, sizes'"
          ></Pagination
        ></el-tab-pane>
        <el-tab-pane label="试卷记录" name="second">
          <vxe-table
            round
            border
            auto-resize
            ...
            :cell-class-name="cellClassName"
            ref="TraningtRank"
            :seq-config="{ startIndex: (this.page2 - 1) * this.limit2 }"
            :loading="loading"
          >
            <vxe-table-column
              field="rank"
            ...
            <vxe-table-column field="ac" title="通过数" max-width="60">
            </vxe-table-column>
            <vxe-table-column
              max-width="100"
              v-for="(exam, index) in trainingExamList"
              :key="index"
            >
              <template v-slot:header>
                {{ exam.title }}({{ exam.totalScore }})
              </template>
              <template v-slot="{ row }">
                <a
                  v-if="row.examSubmissionInfo[exam.id]"
                  @click="getExamHistory(row.examSubmissionInfo[exam.id])"
                  style="color:rgb(87, 163, 243);font-size:14px"
                >
                  {{ row.examSubmissionInfo[exam.id].userScore }}
                </a>
              </template>
            </vxe-table-column>
          </vxe-table>
...
export default {
  ...
  mounted() {
    ...
    if (!this.trainingExamList.length) {
      this.getTrainingExamList2();
    }
    this.trainingID = this.$route.params.trainingID;
    this.getTrainingRankData();
  },
  methods: {
    ...mapActions(["getTrainingProblemList", "getTrainingExamList2"]),
    ...
    getTrainingRankData() {
      ...
      api.getTrainingRank(data).then(
        (res) => {
          let oj = res.data.data.oj;
          let exam = res.data.data.exam;
          this.total = oj.total;
          this.total2 = exam.total;
          this.applyToTable(oj.records, exam.records);
          this.loading = false;
        },
        (err) => {
          this.loading = false;
    ...
    getExamHistory(row) {
      this.$router.push({
        name: "ExamHistory",
        query: { historyId: row.id, repoID: row.repoId },
      });
    },
    ...
    applyToTable(dataRank, dataRank2) {
      ...
      dataRank2.forEach((rank, i) => {
        if (dataRank2[i].gender == "female") {
          dataRank2[i].userCellClassName = "bg-female";
        }
      });
      this.dataRank2 = dataRank2;
    },
  ...
  computed: {
    ...mapState({
      ...
      trainingExamList: (state) => state.training.trainingExamList,
    }),
