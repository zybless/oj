              ...
              >
                <div slot="header" class="panel-title">
                  ...
                  <div class="problem-menu">
                    <span v-if="isAdmin || isShowProblemDiscussion">
                      <el-link
                        type="primary"
                        :underline="false"
                  ...
                  <div class="question-intr">
                    <template v-if="!isCFProblem">
                      <span
                        >{{ $t("m.Time_Limit") }}：
                        {{ problemData.problem.timeLimit }}MS</span
                      ><span style="display:inline-block;width: 20px;"></span>
                      <span
                        >{{ $t("m.Memory_Limit") }}：
                        {{ problemData.problem.memoryLimit }}MB</span
                      ><br />
                    </template>
...
export default {
  ...
  methods: {
    getSubStatus() {
      ...
      api.getSubmissionList(20, params).then((res) => {
        if (res.data.data.records.length > 0) {
          // 获取所有提交记录中的最高分
          const maxScore = Math.max(...res.data.data.records.map(record => record.score));
          // 如果最高分大于等于50，则显示讨论
          this.isShowProblemDiscussion = maxScore >= this.problemData.problem.solutionUnlockRatio;
          console.log(this.problemData.problem.solutionUnlockRatio)
        }
      });
    },
    ...
    resizeWatchHeight() {
      try {
        ...
        if (this.showProblemHorizontalMenu) {
          let footerMenuHeight = document.getElementById("problem-footer")
            .offsetHeight;
          problemLeftHight = problemLeftHight - footerMenuHeight;
        }
        let jsRHeaderHeight = document.getElementById("js-right-header")
          .offsetHeight;
        let jsRBottomHeight = document.getElementById("js-right-bottom")
          .offsetHeight;
  ...
  watch: {
    ...
    'problemData.problem.solutionUnlockRatio': {
      handler(newVal) {
        if (newVal !== undefined) {
          this.getSubStatus();
        }
