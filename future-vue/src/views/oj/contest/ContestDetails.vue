              ...
              <div
                v-if="
                  contestStatus == CONTEST_STATUS.RUNNING &&
                    startTime &&
                    contest.userDuration &&
                    contest.userDuration > 0
                "
                style="float: right;font-size: 18px;"
              >
                <span v-if="countdown2">剩余时间:{{ countdown2 }}</span>
                <span v-else-if="startTime">比赛已结束</span>
                <span v-else>比赛未开始</span>
              </div>
            </el-col>
          </el-row>
      ...
      <el-tabs @tab-click="tabClick" v-model="route_name">
        <el-tab-pane name="ContestDetails" lazy>
          ...
          >
            ...
            <span
              v-if="
                contest.userDuration &&
                  contest.userDuration > 0 &&
                  contestStatus == CONTEST_STATUS.SCHEDULED
              "
            ></span>
            <p v-else class="password-form-tips">
              {{ $t("m.To_Enter_Need_Password") }}
            </p>
            <p
              class="password-form-tips"
              v-if="
                contest.userDuration &&
                  contest.userDuration > 0 &&
                  contestStatus == CONTEST_STATUS.SCHEDULED
          ...
          <div slot="header">
            ...
            <p
              class="password-form-tips"
              v-if="contest.userDuration && contest.userDuration > 0"
            >
              该比赛限时{{ this.contest.userDuration / 60 }}分钟
            </p>
            <el-button
              type="primary"
              @click="enterContest"
...
import { startContest, getStartTime } from "@/common/newApi/contest/index";
export default {
  name: "ContestDetails",
  components: {
    Markdown,
  ...
  methods: {
    ...
    getContestStartTime() {
      if (this.contest.userDuration && this.contest.userDuration > 0) {
        getStartTime({ cid: this.contestID }).then((res) => {
    ...
    enterContest(){
      if (this.contest.userDuration && this.contest.userDuration > 0) {
        this.$confirm("进入后开始计时，是否开始", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        }).then(() => {
          startContest(this.contestID).then(() => {
            this.getContestStartTime();
          });
        });
      } else {
