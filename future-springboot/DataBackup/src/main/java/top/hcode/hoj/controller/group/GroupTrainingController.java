...
public class GroupTrainingController {
    ...
    @PostMapping("/add-training-from-public")
    public CommonResult<Void> addTrainingFromPublic(@RequestParam(value = "tidList", required = true) List<Long> tidList,
                                                    @RequestParam(value = "groupId", required = true) Long groupId) {
        return groupTrainingService.addTrainingFromPublic(tidList, groupId);
    }
