...
public class AdminUserManager {
    ...
    public void editUser(AdminEditUserDTO adminEditUserDto) throws StatusFailException {
        ...
        Float oiCoin = adminEditUserDto.getOiCoin();
        ...
        if (type == 1000 && !root) {
            throw new StatusFailException("只有超级管理员可以修改超级管理员权限！");
        }
    ...
    public void addVip(String uuid, String date) throws StatusFailException {
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        Date parseDate;
        try {
            parseDate = simpleDateFormat.parse(date);
        } catch (ParseException e) {
            throw new StatusFailException("日期错误");
        }

        Date curDate = new Date();

        if (curDate.after(parseDate)) {
            throw new StatusFailException("日期错误");
        }

        UserInfo user = userInfoEntityService.getById(uuid);
        if(user == null) {
            throw new StatusFailException("用户不存在");
        }

        if (user.getIsVip() != null && user.getIsVip()) {
            user.setVipExpiredTime(parseDate);
        } else {
            user.setIsVip(true);
    ...
    public void removeVip(String uuid) throws StatusFailException {
        UserInfo user = userInfoEntityService.getById(uuid);
        if(user == null) {
            throw new StatusFailException("用户不存在");
        }
        if (user.getIsVip() != null && !user.getIsVip()) {
            throw new StatusFailException("用户不是VIP");
        }
        user.setIsVip(false);
        userInfoEntityService.updateById(user);
        userRoleEntityService.deleteVip(user.getUuid());
    }
}