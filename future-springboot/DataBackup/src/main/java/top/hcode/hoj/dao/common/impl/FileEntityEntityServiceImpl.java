...
public class FileEntityEntityServiceImpl extends ServiceImpl<FileMapper, File> implements FileEntityService {
    ...
    @Override
    public int updateFileToDeleteByCidAndType(Long courseId, String type) {
        return fileMapper.updateFileToDeleteByCidAndType(courseId, type);
    }
    ...
    @Override
    public List<List<String>> getExamRankExcelHead(Integer problemSize) {
        List<List<String>> headList = new LinkedList<>();
    ...
    @Override
    public List<List<Object>> changeExamRankToExcelRowList(List<ExamRankVO> examRankVOList) {
        List<List<Object>> allRowDataList = new LinkedList<>();
        for (ExamRankVO examRankVO : examRankVOList) {
            List<Object> rowData = new LinkedList<>();

            rowData.add(examRankVO.getRank());
            rowData.add(examRankVO.getUsername());
            rowData.add(examRankVO.getRealname());
            rowData.add(examRankVO.getUserScore());
            rowData.add(examRankVO.getSchool());

            Long historyId = examRankVO.getHistoryId();
            QueryWrapper<ExamHistoryDetail> examHistoryQueryWrapper = new QueryWrapper<>();
            examHistoryQueryWrapper.eq("history_id", historyId);
            examHistoryQueryWrapper.select("is_correct");
            List<ExamHistoryDetail> examHistoryDetailList = examHistoryDetailEntityService.list(examHistoryQueryWrapper);

            for (ExamHistoryDetail examHistoryDetail : examHistoryDetailList) {
