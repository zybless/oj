...
public class ContestManager {
    ...
    @Autowired
    private ContestStartStatusEntityService contestStartStatusEntityService;
    ...
    public void toRegisterContest(RegisterContestDTO registerContestDto) throws StatusFailException, StatusForbiddenException {
        ...
        if (contest.getIsVip() != null && contest.getIsVip().equals(1) && !userRolesVo.getIsVip()) {
            throw new StatusForbiddenException("该比赛只有VIP能参加");
        }
    ...
    public ProblemInfoVO getContestProblemDetails(Long cid, String displayId, Boolean isContainsContestEndJudge) throws StatusFailException, StatusForbiddenException, StatusNotFoundException {
        ...
        return new ProblemInfoVO(problem, tags, languagesStr, problemCount, null, LangNameAndCode);
    }
    ...
    public IPage getContestRank(ContestRankDTO contestRankDto) throws StatusFailException, StatusForbiddenException {
        ...
        Integer sortMethod = contestRankDto.getSortMethod();
    ...
    public void startContest(Long cid) throws StatusForbiddenException, StatusFailException {
        AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();
        Contest contest = contestEntityService.getById(cid);
        ...
        QueryWrapper<ContestStartStatus> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("cid", cid).eq("uid", userRolesVo.getUid());
        if(!contestStartStatusEntityService.list(queryWrapper).isEmpty()) {
            throw new StatusFailException("您已点击过开始！");
        }

        ContestStartStatus contestStartStatus = new ContestStartStatus();
        contestStartStatus.setCid(cid);
        contestStartStatus.setUid(userRolesVo.getUid());
        contestStartStatus.setStartTime(new Date());

        contestStartStatusEntityService.save(contestStartStatus);
    }

    public Date getContestStartTime(Long cid) throws StatusFailException {
        AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();
        QueryWrapper<ContestStartStatus> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("cid", cid).eq("uid", userRolesVo.getUid());
        List<ContestStartStatus> contestStartStatusList = contestStartStatusEntityService.list(queryWrapper);
        if(contestStartStatusList == null || contestStartStatusList.isEmpty()) {
            return null;
        }
        return contestStartStatusList.get(contestStartStatusList.size() - 1).getStartTime();
    }
}