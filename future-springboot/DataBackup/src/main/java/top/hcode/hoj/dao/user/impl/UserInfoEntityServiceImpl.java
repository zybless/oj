...
public class UserInfoEntityServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo> implements UserInfoEntityService {
    ...
    @Override
    public List<String> getAdminUidList() {

        String cacheKey = Constants.Account.ADMIN_UID_LIST_CACHE.getCode();
        List<String> adminUidList = (List<String>) redisUtils.get(cacheKey);
        if (adminUidList == null) {
            adminUidList = userInfoMapper.getAdminUidList();
            redisUtils.set(cacheKey, adminUidList, 12 * 3600);
        }
        return adminUidList;
    }
