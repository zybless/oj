...
public class AdminProblemController {
    ...
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "problem_admin", "admin", "teacher"}, logical = Logical.OR)
    ...
    @GetMapping("/get-io-record")
    @RequiresAuthentication
    @RequiresRoles(value = {"root"}, logical = Logical.OR)
    public CommonResult<IPage<ProblemIoRecord>> getProblemIoRecord(@RequestParam(value = "limit", required = false) Integer limit,
                                                                   @RequestParam(value = "currentPage", required = false) Integer currentPage,
                                                                   @RequestParam(value = "keywords", required = false) String keywords) {
        return adminProblemService.getProblemIoRecord(limit, currentPage, keywords);
    }

    @PostMapping(value = "/batch-zip", consumes = "multipart/form-data")
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "admin", "teacher"}, logical = Logical.OR)
    public CommonResult<Void> addProblemBatchWithZip(@RequestPart(value = "file") MultipartFile[] files,
                                                     @RequestParam(value = "id") String prefix,
                                                     @RequestParam(value = "num") Integer number) {
//        MultipartFile files[] = null;
        return adminProblemService.addProblemBatchWithZip(files, prefix, number);
    }
