...
import top.hcode.hoj.dao.contest.ContestStartStatusEntityService;
...
public class BeforeDispatchInitManager {
    ...
    public void initContestSubmission(Long cid, String displayId, AccountProfile userRolesVo, Judge judge) throws StatusNotFoundException, StatusForbiddenException {
        ...
        // 是否单独计时
        if (contest.getUserDuration() != null && contest.getUserDuration() > 0) {
            ...
            if (contestStartStatusList == null || contestStartStatusList.isEmpty()) {
                throw new StatusForbiddenException("您还未点击开始比赛。");
            } else {
                Date date = new Date();
                if ((date.getTime() - contestStartStatusList.get(contestStartStatusList.size() - 1).getStartTime().getTime()) / 1000 > contest.getUserDuration()) {
                    throw new StatusForbiddenException("您的比赛时间已结束！");
                }
            }
        }
