...
public class AdminProblemServiceImpl implements AdminProblemService {
    ...
    @Override
    public CommonResult<IPage<ProblemIoRecord>> getProblemIoRecord(Integer limit, Integer currentPage, String keywords) {
        IPage<ProblemIoRecord> problemIoRecord = adminProblemManager.getProblemIoRecord(limit, currentPage, keywords);
        return CommonResult.successResponse(problemIoRecord);
    }

    @Override
    public CommonResult<Void> addProblemBatchWithZip(MultipartFile[] files, String prefix, Integer number) {
        try {
            adminProblemManager.addProblemBatchWithZip(files, prefix, number);
            return CommonResult.successResponse("导入成功");
        } catch (StatusFailException | StatusSystemErrorException e) {
            return CommonResult.errorResponse(e.getMessage());
        }
