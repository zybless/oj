    ...
    public IPage<Discussion> getDiscussionList(Integer limit,
        ...
        if (!StringUtils.isEmpty(pid)) {
            Problem problem = problemEntityService.lambdaQuery()
                    ...
                    .one();
            Judge judge = judgeEntityService.lambdaQuery()
                    .eq(Judge::getPid, problem.getId())
                    .eq(Judge::getUid, userRolesVo.getUid())
                    .orderByDesc(Judge::getScore)
                    .last("limit 1")
                    .one();

//            //TODO:若无空指针可删除
//            if (problem.getSolutionUnlockRatio() == null) {
//                problem.setSolutionUnlockRatio(BigDecimal.valueOf(50));
//            }

            if ((judge == null || judge.getScore() < problem.getSolutionUnlockRatio().floatValue()) && !isAdmin) {
                throw new StatusForbiddenException("对不起，您没有权限访问该题题解！（题目分数需要达到" + problem.getSolutionUnlockRatio().floatValue() + "分）");
            }

            discussionQueryWrapper.eq("pid", pid);

        } else {
            discussionQueryWrapper.isNull("pid");
...
//        }
        ...
        if (discussionVo.getGid() != null && discussionVo.getPid() == null) {
        ...
        if (discussionVo.getPid() != null && !isAdmin) {
            Problem problem = problemEntityService.lambdaQuery()
                    .eq(Problem::getProblemId, discussionVo.getPid())
                    .one();
            Judge judge = judgeEntityService.lambdaQuery()
                    .eq(Judge::getPid, problem.getId())
                    .eq(Judge::getUid, userRolesVo.getUid())
