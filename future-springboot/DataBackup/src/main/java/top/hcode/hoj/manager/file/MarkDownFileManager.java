...
public class MarkDownFileManager {
    ...
    public Map<Object, Object> uploadMDImg(MultipartFile image, Long gid) throws StatusFailException, StatusSystemErrorException, StatusForbiddenException {
        ...
        try {
            //将文件保存指定目录
            image.transferTo(FileUtil.file(new File(Constants.File.MARKDOWN_FILE_FOLDER.getPath()).getAbsolutePath() + File.separator + filename));
        } catch (Exception e) {
    ...
    public Map<Object, Object> uploadMd(MultipartFile file, Long gid) throws StatusFailException, StatusSystemErrorException, StatusForbiddenException {
        ...
        try {
            //将文件保存指定目录
            file.transferTo(FileUtil.file(new File(Constants.File.MARKDOWN_FILE_FOLDER.getPath()).getAbsolutePath() + File.separator + filename));
        } catch (Exception e) {
            log.error("文件上传异常-------------->", e);
