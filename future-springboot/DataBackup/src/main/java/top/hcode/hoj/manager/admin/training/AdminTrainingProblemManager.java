...
public class AdminTrainingProblemManager {
    ...
    public IPage<ExamRepo> getExamList(Integer limit, Integer currentPage, String keyword, Long tid) {
        if (currentPage == null || currentPage < 1) currentPage = 1;
        if (limit == null || limit < 1) limit = 10;
        ...
        trainingExamQueryWrapper.eq("tid", tid).orderByAsc("display_id");

        List<Long> eidList = new LinkedList<>();

        List<TrainingExam> trainingExamList = trainingExamEntityService.list(trainingExamQueryWrapper);
        HashMap<Long, TrainingExam> trainingExamMap = new HashMap<>();
        trainingExamList.forEach(trainingExam -> {
            if (!trainingExamMap.containsKey(trainingExam.getEid())) {
                trainingExamMap.put(trainingExam.getEid(), trainingExam);
            }
            eidList.add(trainingExam.getEid());
        });
        ...
        QueryWrapper<ExamRepo> examQueryWrapper = new QueryWrapper<>();
        examQueryWrapper.in(eidList.size() > 0, "id", eidList);
        if (keyword != null && !keyword.equals("")) {
            examQueryWrapper.like("title", keyword);
        }

        IPage<ExamRepo> examIPage = examRepoEntityService.page(iPage, examQueryWrapper);

        return examIPage;
    }

    public void updateExam(TrainingExam trainingExam) throws StatusFailException {
        boolean isOk = trainingExamEntityService.saveOrUpdate(trainingExam);
    ...
    public void deleteExam(Long eid, Long tid) throws StatusFailException {
        boolean isOk = false;
        //  训练id不为null，表示就是从比赛列表移除而已
        if (tid != null) {
            QueryWrapper<TrainingExam> trainingExamQueryWrapper = new QueryWrapper<>();
            trainingExamQueryWrapper.eq("tid", tid).eq("eid", eid);
            isOk = trainingExamEntityService.remove(trainingExamQueryWrapper);
        } else {
             /*
                problem的id为其他表的外键的表中的对应数据都会被一起删除！
              */
//            isOk = examEntityService.removeById(eid);
            throw new StatusFailException("需要指定题单！");
        }
        ...
        if (isOk) { // 删除成功
            ...
            AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();
            if (tid == null) {
//                FileUtil.del(Constants.File.TESTCASE_BASE_FOLDER.getPath() + File.separator + "exam_" + eid);
//                log.info("[{}],[{}],tid:[{}],pid:[{}],operatorUid:[{}],operatorUsername:[{}]",
//                        "Admin_Training", "Delete_Exam", tid, eid, userRolesVo.getUid(), userRolesVo.getUsername());
            } else {
                log.info("[{}],[{}],tid:[{}],pid:[{}],operatorUid:[{}],operatorUsername:[{}]",
                        "Admin_Training", "Remove_Exam", tid, eid, userRolesVo.getUid(), userRolesVo.getUsername());
            }
            // 更新训练最近更新时间
            UpdateWrapper<Training> trainingUpdateWrapper = new UpdateWrapper<>();
            trainingUpdateWrapper.set("gmt_modified", new Date())
    ...
    public void addExamFromPublic(TrainingExamDTO trainingExamDTO) throws StatusFailException {
        Long eid = trainingExamDTO.getEid();
        Long tid = trainingExamDTO.getTid();
        String displayId = trainingExamDTO.getDisplayId();

        QueryWrapper<TrainingExam> trainingExamQueryWrapper = new QueryWrapper<>();
        trainingExamQueryWrapper.eq("tid", tid)
                .and(wrapper -> wrapper.eq("eid", eid)
                        .or()
                        .eq("display_id", displayId));
        TrainingExam trainingExam = trainingExamEntityService.getOne(trainingExamQueryWrapper, false);
        if (trainingExam != null) {
            throw new StatusFailException("添加失败，该试卷已添加或者试卷的训练展示ID已存在！");
        }

        TrainingExam newTExam = new TrainingExam();
        boolean isOk = trainingExamEntityService.saveOrUpdate(newTExam
                .setTid(tid).setEid(eid).setDisplayId(displayId));
        if (isOk) { // 添加成功
            ...
            AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();
            log.info("[{}],[{}],tid:[{}],pid:[{}],operatorUid:[{}],operatorUsername:[{}]",
                    "Admin_Training", "Add_Public_Exam", tid, eid, userRolesVo.getUid(), userRolesVo.getUsername());

            // 异步地同步用户对该题目的提交数据
            adminTrainingRecordManager.syncAlreadyRegisterUserRecord(tid, eid, newTExam.getId());
        } else {
            throw new StatusFailException("添加失败！请检查是否重复！");
        }
    }
}