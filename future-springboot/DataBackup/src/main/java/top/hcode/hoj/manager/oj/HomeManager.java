...
//        return contestEntityService.getWithinNext14DaysContests();
        return contestEntityService.getRecentContest(7, 7);
    }
     ...
     * @return
     */
    public RankingListVO getRankingList() {
        RankingListVO rankingListVO = (RankingListVO) redisUtils.get(Constants.Training.RANKING_LIST_KEY.getValue());
        AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();
        if (rankingListVO != null && userRolesVo != null) {
            for (List<RankingListDetailVO> rankingListDetailVOS : rankingListVO.getRankingListDetailVOList()) {
                OptionalInt index = IntStream.range(0, rankingListDetailVOS.size())
                        .filter(i -> userRolesVo.getUid().equals(rankingListDetailVOS.get(i).getUid()))
                        .findFirst();

                if (index.isPresent()) {
                    rankingListVO.getMyRank().add(index.getAsInt());
                } else {
                    rankingListVO.getMyRank().add(-1);
                }
            }
            if(userRolesVo.getRealname() != null && !userRolesVo.getRealname().isEmpty()) {
                rankingListVO.setUsername(userRolesVo.getRealname());
            } else {
                rankingListVO.setUsername(userRolesVo.getUsername());
            }
            Map<String, List<Integer>> acNumMap = JSONUtil.toBean((String)redisUtils.get(Constants.Training.USER_ACNUM_KEY.getValue()),
                    new cn.hutool.core.lang.TypeReference<Map<String, List<Integer>>>() {}, false);
            rankingListVO.setAcNum(acNumMap.get(userRolesVo.getUid()));
        }
    ...
    public MonthRankingVO getMonthRanking() {
        MonthRankingVO monthRankingVO = (MonthRankingVO)redisUtils.get(Constants.Training.MONTH_RANKING_KEY.getValue());
        return monthRankingVO;
    }
