...
public class ProblemManager {
    ...
    public UserGroupProblemStatusVO getUserGroupProblemStatus() {
        // 需要获取一下该token对应用户的数据
        AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();

        UserGroupProblemStatusVO userGroupProblemStatusVO = new UserGroupProblemStatusVO();

        // 获取用户加入的班级数据
        QueryWrapper<GroupMember> groupMemberQueryWrapper = new QueryWrapper<>();
        groupMemberQueryWrapper.eq("uid", userRolesVo.getUid());
        List<GroupMember> groupMemberList = groupMemberEntityService.list(groupMemberQueryWrapper);

        HashMap<Long, HashMap<Long, HashMap<Long, Object>>> groupTrainingProblemMap = new HashMap<>();

        for (GroupMember groupMember : groupMemberList) {
            Group group;
            try {
                group = groupManager.getGroup(groupMember.getGid());
            } catch (Exception e) {
                continue;
            }
            userGroupProblemStatusVO.getGroupDetail().put(group.getId(), group);

            // 获取班级里的题单
            QueryWrapper<Training> trainingQueryWrapper = new QueryWrapper<>();
            trainingQueryWrapper.eq("gid", groupMember.getGid());
            List<Training> trainingList = trainingEntityService.list(trainingQueryWrapper);

            HashMap<Long, HashMap<Long, Object>> trainingProblemMap = new HashMap<>();

            for (Training training : trainingList) {
                TrainingVO trainingVO;
                try {
                    trainingVO = trainingManager.getTraining(training.getId());
                } catch (Exception e) {
                    continue;
                }
                userGroupProblemStatusVO.getTrainingDetail().put(trainingVO.getId(), trainingVO);

                // 获取题单里面的题目
                List<ProblemVO> trainingProblemList = trainingProblemEntityService.getTrainingProblemList(training.getId());
                if (trainingProblemList.size() == 0) {
                    continue;
                }

                HashMap<Long, Object> problemMap = new HashMap<>();

                List<Long> problemIdList = new ArrayList<>();
                for (ProblemVO trainingProblem : trainingProblemList) {
                    problemIdList.add(trainingProblem.getPid());
                    userGroupProblemStatusVO.getProblemDetail().put(trainingProblem.getPid(), trainingProblem);
                }

                QueryWrapper<Judge> queryWrapper = new QueryWrapper<>();
                queryWrapper.select("distinct pid,status,submit_time,score")
                        .in("pid", problemIdList)
                        .eq("uid", userRolesVo.getUid())
                        .orderByDesc("submit_time");

                queryWrapper.eq("cid", 0);
                queryWrapper.eq("gid", groupMember.getGid());
                ...
                for (Judge judge : judges) {
                    ...
                    if (judge.getStatus().intValue() == Constants.Judge.STATUS_ACCEPTED.getStatus()) {
                        // 如果该题目已通过，则强制写为通过（0）
                        temp.put("status", Constants.Judge.STATUS_ACCEPTED.getStatus());
                        problemMap.put(judge.getPid(), temp);
                    } else if (!problemMap.containsKey(judge.getPid())) {
                        // 还未写入，则使用最新一次提交的结果
                        temp.put("status", judge.getStatus());
                        problemMap.put(judge.getPid(), temp);
                    }
                ...
                for (Long pid : problemIdList) {
                    // 如果是比赛的题目列表状态
                    if (!problemMap.containsKey(pid)) {
                        HashMap<String, Object> temp = new HashMap<>();
                        temp.put("status", Constants.Judge.STATUS_NOT_SUBMITTED.getStatus());
                        problemMap.put(pid, temp);
                    }
                }

                trainingProblemMap.put(trainingVO.getId(), problemMap);
            }
            groupTrainingProblemMap.put(group.getId(), trainingProblemMap);
        }
        userGroupProblemStatusVO.setStatusMap(groupTrainingProblemMap);

        return userGroupProblemStatusVO;
    }
    ...
    public ProblemInfoVO getProblemInfo(String problemId, Long gid) throws StatusNotFoundException, StatusForbiddenException {
        ...
        UserInfo userInfo = userInfoEntityService.getById(userRolesVo.getUid());

//        if (!problem.getDifficulty().equals(0) && !userInfo.getIsVip()) {
//            throw new StatusForbiddenException("非VIP只能访问简单题");
//        }
        ...
        if (CollectionUtil.isNotEmpty(lidList)) {
            Collection<Language> allLanguages = languageEntityService.listByIds(lidList);
            // 过滤不要的语言
            List<Language> languages = new ArrayList<>();
            Set<String> languageSet = Stream.of(
                    "C++14",
                    "C++14(O2)",
                    "C++17",
                    "C++17(O2)",
            ...
            for (Language language : allLanguages) {
                if (languageSet.contains(language.getName())) {
//                    if (language.getName().equals("C++")) {
//                        language.setName("C++14");
//                    }
//                    if (language.getName().equals("C++ With O2")) {
//                        language.setName("C++14(O2)");
//                    }
//                    if (language.getName().equals("C++ 17")) {
//                        language.setName("C++17");
//                    }
//                    if (language.getName().equals("C++ 17 With O2")) {
//                        language.setName("C++17(O2)");
//                    }
...
//        judgeQueryWrapper.eq("uid", userRolesVo.getUid())
//                .eq("pid", problem.getId())
//                .orderByDesc("submit_time") // 根据提交时间降序排序
...
//        }
        ...
        return new ProblemInfoVO(problem, tags, languagesStr, problemCount, code, LangNameAndCode);
    }
