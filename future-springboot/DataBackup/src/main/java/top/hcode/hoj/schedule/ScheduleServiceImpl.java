...
import top.hcode.hoj.utils.PrometheusQueryUtils;
...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    @Autowired
    private PrometheusQueryUtils prometheusQueryUtils;
    ...
    public void deleteVIP() {
        ...
        for (UserInfo userInfo : userInfoEntityService.list()) {
            if(userInfo.getIsVip() != null && userInfo.getIsVip().equals(true)) {
                if(userInfo.getVipExpiredTime().before(curDate)) {
                    userInfo.setIsVip(false);
                    userInfoEntityService.updateById(userInfo);
                    userRoleEntityService.deleteVip(userInfo.getUuid());
                }
            }
        }
    }
...
//        Object ongoingExam = redisUtils.get("OngoingExam");
//        if (ongoingExam == null) {
//            return;
//        }
//
//        List<Exam> ongoingExamList = (List<Exam>) ongoingExam;
//        for (Exam exam : ongoingExamList) {
//            // 判断是否超时
...
//    @Scheduled(cron = "0/5 * * * * *")
    ...
    public void checkDiskSpace() {
        ...
        for (ServiceInstance serviceInstance : serviceInstances) {
            DiskInfo diskInfo = prometheusQueryUtils.queryDiskInfo("http://" + serviceInstance.getUri().getHost());
            if (diskInfo != null && diskInfo.getUsagePercentage() > 90) {
                emailManager.sendDiskInfo(diskInfo, serviceInstance.getUri().getHost(), serviceInstance.getServiceId());
            }
        }
    ...
    public void queryJudgeServerInfo() {
        List<JSONObject> judgeServiceInfo = configManager.getJudgeServiceInfo();
        redisUtils.set(Constants.RedisKey.JUDGE_SERVER_INFO.getKey(), judgeServiceInfo);
    }
