...
public interface FileMapper extends BaseMapper<File> {
    ...
    @Update("UPDATE `file` SET `delete` = 1 WHERE `cid` = #{courseId} AND `type` = #{type}")
    int updateFileToDeleteByCidAndType(@Param("courseId") Long courseId, @Param("type") String type);
