...
public class JudgeManager {
    ...
    public SubmissionInfoVO getSubmission(Long submitId) throws StatusNotFoundException, StatusAccessDeniedException {
        ...
        boolean isTeacher = SecurityUtils.getSubject().hasRole("teacher"); // 是否为超级管理员
        boolean isAdmin = SecurityUtils.getSubject().hasRole("admin"); // 是否为超级管理员
        ...
        if (judge.getCid() != 0) {
            ...
            if (!isRoot && !isTeacher && !isAdmin && !userRolesVo.getUid().equals(contest.getUid())
            ...
            if (!isRoot && !isTeacher && !isAdmin && !isProblemAdmin && judge.getCode() != null) {
                try {
                    accessValidator.validateAccess(HOJAccessEnum.HIDE_NON_CONTEST_SUBMISSION_CODE);
                } catch (AccessException e) {
                    judge.setCode("Because the super administrator has enabled " +
