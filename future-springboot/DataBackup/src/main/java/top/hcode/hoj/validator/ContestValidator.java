...
public class ContestValidator {
    ...
    public void validateContestAuth(Contest contest, AccountProfile userRolesVo, Boolean isRoot) throws StatusFailException, StatusForbiddenException {
        ...
        Long gid = contest.getGid();
        // 若是比赛管理者
        if (isContestAdmin || (contest.getIsGroup() && groupValidator.isGroupRoot(userRolesVo.getUid(), gid)) || isTeacher || isAdmin) {
            return;
        }
        ...
        if (contest.getStatus().intValue() != Constants.Contest.STATUS_RUNNING.getCode() &&
            ...
            if (contest.getUserDuration() != null && contest.getUserDuration() > 0) {
                ...
                queryWrapper.eq("cid", contest.getId()).eq("uid", userRolesVo.getUid());
                ...
                if (contestStartStatusList == null || contestStartStatusList.isEmpty()) {
                    throw new StatusFailException("对不起，您还未点击开始作答。");
                }
            }
        }
