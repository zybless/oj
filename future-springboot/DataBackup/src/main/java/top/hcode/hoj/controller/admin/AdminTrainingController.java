...
public class AdminTrainingController {
    ...
    @DeleteMapping("")
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "teacher"}, logical = Logical.OR) // 只有超级管理员能删除训练
    ...
    @PostMapping("")
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "teacher", "teacher"}, logical = Logical.OR)
    ...
    @PutMapping("/problem")
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "problem_admin", "teacher"}, logical = Logical.OR)
    ...
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "admin", "problem_admin", "teacher"}, logical = Logical.OR)
    public CommonResult<IPage<ExamRepo>> getExamList(@RequestParam(value = "limit", required = false) Integer limit,
                                                     @RequestParam(value = "currentPage", required = false) Integer currentPage,
                                                     @RequestParam(value = "keyword", required = false) String keyword,
                                                     @RequestParam(value = "tid", required = true) Long tid) {
        return adminTrainingProblemService.getExamList(limit, currentPage, keyword, tid);
    }

    @PutMapping("/exam")
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "problem_admin", "teacher"}, logical = Logical.OR)
    public CommonResult<Void> updateExam(@RequestBody TrainingExam trainingExam) {
        return adminTrainingProblemService.updateExam(trainingExam);
    }

    @DeleteMapping("/exam")
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "problem_admin", "teacher"}, logical = Logical.OR)
    public CommonResult<Void> deleteExam(@RequestParam("eid") Long eid,
                                            @RequestParam(value = "tid", required = false) Long tid) {

        return adminTrainingProblemService.deleteExam(eid, tid);
    }

    @PostMapping("/add-exam-from-public")
    @RequiresAuthentication
    @RequiresRoles(value = {"root", "teacher"}, logical = Logical.OR)
    public CommonResult<Void> addExamFromPublic(@RequestBody TrainingExamDTO trainingExamDTO) {
        return adminTrainingProblemService.addExamFromPublic(trainingExamDTO);
    }
